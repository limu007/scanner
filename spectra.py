...
lor_si_low2=[ 3.351354,[[3.22408445e+02, 3.66550689e+00, 8.94634583e-02], [41.29945655,  6.05035883,  0.07599436]]]
...
def dbload(name='SiO2_gl',freq=None,connection="http"):
    from numpy import loadtxt,arange,array,iterable
    if iterable(freq):
...
def extrema(x,b=None,lbin=0,poly=0,ret_all=False,check_bins=True,msplit=False,retype='nd',
    ...
    from numpy import polyfit,polyval,arange,array,iterable
    if not iterable(b): b=arange(len(x))
    ...
    if extsel!=None:
        if extsel<0: # reporting quality of the fit
            btops,mtops,ctops=ext_ran_fit(atops,x,b,-extsel,True,True,alim=-curv_limit[0],loud=loud)
            bbots,mbots,cbots=ext_ran_fit(abots,x,b,-extsel,True,True,alim=curv_limit[1],loud=loud)
            if ampmeas: return bbots,btops,mbots,mtops,cbots,ctops
            return bbots,btops,cbots,ctops
        else: 
            btops,mtops=ext_ran_fit(atops,x,b,extsel,False,True,alim=-curv_limit[0],loud=loud)
            bbots,mbots=ext_ran_fit(abots,x,b,extsel,False,True,alim=curv_limit[1],loud=loud)            
...
sin_fin=None #which function is used to fit quasi sinusoidal profiles
#one example is sin_com function below, other is extra_fun
#should 
par_con=[]
...
def sin_com(p,x,y=None,np=None,nord=2,inv=False,cent=False):
...
def fitting(x,y,p0=None,prange=None,bounds=None,loud=1,fit_mode=1,refr_mode=None,lbin=0,nprof=None,nord=2):
    ...
    if p0=='last': p0=last_pars
        ...
        p0+=list(ad)
        if refr_mode=='poly':p0+=[0,0] # two relative cauchy parameters
        if loud>0:
...
def extra_fun(y,per,exten=0,p=None):
    ...
    from numpy import sin,iterable
    if exten==1: 
